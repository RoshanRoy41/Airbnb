...
document.addEventListener('DOMContentLoaded', () => {
  ...
  const checkin:string | null= getUrlParameter('chkin');
  const checkout:string | null = getUrlParameter('chkout');
  ...
  const checkinElement = document.querySelector('.card-text.checkin') as HTMLElement | null;
  const checkoutElement = document.querySelector('.card-text.checkout') as HTMLElement | null;
